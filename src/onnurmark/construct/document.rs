...
use crate::onnurmark::subtokenize::divide_events;
use crate::onnurmark::tokenizer::{Container, ContainerState, Tokenizer};
use crate::onnurmark::util::skip;
...
pub fn container_existing_after(tokenizer: &mut Tokenizer) -> State {
    ...
    if container.kind == Container::Environment {
        ...
        return State::Retry(if tokenizer.tokenize_state.environment_opened {
            StateName::DocumentContainerExistingBefore
        } else {
            StateName::DocumentContainerNewBefore
        });
    }
